...
clear
set more off
global data "E:\GoogleDrive\Data\Ldvl\"
global temp "F:\dropbox\temp\ldvl"

use "$data\lfs_2015_q1.dta", clear
	gen ky_3t =1
	gen w_p_q = cal_weight_final 
tempfile file_q1
save `file_q1', replace
use "$data\lfs_2015_q2.dta", clear
	gen ky_3t =2
	gen w_p_q = cal_weigh_final
	gen thieuvieclam = thieuvl
tempfile file_q2
save `file_q2', replace
use "$data\lfs_2015_q3.dta", clear
	gen ky_3t =3
	gen w_p_q = cal_weigh
tempfile file_q3
save `file_q3', replace

use "$data\lfs_2015_q4.dta", clear
	gen ky_3t =4
	gen w_p_q = cal_weigh
append using `file_q1'
...
// Giíi tÝnh
gen gender = c3
label define gender 1"Nam" 2 "Nò" 99"Missing"
...
*/
gen underemployment = thieuvieclam
replace thieuvieclam =0 if ky_3t ==4
...
do "F:\Dropbox\dofile\ldvl\support_ldvl2015.do"
...
do "F:\Dropbox\dofile\ldvl\informal_2015.do"

// Recode gi¸ trÞ missing b»ng 99 vµ g¸n nh·n cho biÕn
local namelist bhxh region6 regHN_HCM agegroup5 marital_status gender train train2 train3 train0 employment  ///
...
prev_economic_sector prev_occup1 lydokolv hoatdong hdld trongtuoi hinhthuctimviec migration migration_reasons reg_migration informal prod_uni
...
label values `name' `name'
		...
		* gen weight = w_p
replace underemployment = 0 if ky_3t ==4
...
save "$temp\ldvl_2015_q1-4.dta", replace
