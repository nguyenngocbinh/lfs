...
gen employment = 1 if  c57 ==1 
replace employment = 2 if  c57 == 2 // that nghiep
// BiÕn thÊt nghiÖp ng­îc víi biÕn cã viÖc lµm
gen unemployment = 1 if  c57 ==2
replace unemployment = 2 if  c57 ==1

recode c38 c55 (-1=0)
egen weekhour = rsum(c38 c55) // Tæng sè giê lµm viªc/ 1 tuÇn

// T¹o biÕn thiÕu viÖc lµm
gen underemployment =  1 if c41==1 & c40 !=.
...
gen lydokolv = c20
recode lydokolv (3=99)
recode lydokolv (1=3)
recode lydokolv (2=1)
recode lydokolv (4=2)
recode lydokolv 5=4 9=4 99=4
recode lydokolv .=4
...
gen wage = 0 // TiÒn l­¬ng trong 7 ngµy * 4

gen income = c44 // TiÒn l­¬ng trong 7 ngµy * 4
...
gen tdvh = .
local namelist region6 age agegroup5 marital_status gender train employment occup1 tdvh /// 
unemployment underemployment economic_sector indus1 indus2 labour_force employment_status /// 
ttnt nganh_N_C_D train2 dantoc weekhour bhxh lydokolv hdld
...
gen weight = w_ng
order `namelist' weight tinh unemployment_rate wage income
save $temp\ldvl_2007.dta, replace
