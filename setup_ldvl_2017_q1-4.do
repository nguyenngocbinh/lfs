...
First : 17/05/2016
Updated:  10/3/2017 : add quarter 4
...
use "$data\lfs_2017_q1.dta", clear
	gen ky_3t =1
	gen w_p_q = weigh 
	rename c6 c6a
tempfile file_q1
...
use "$data\lfs_2017_q2.dta", clear
	gen ky_3t =2
	gen w_p_q = weigh_final
	rename c6 c6a
tempfile file_q2
save `file_q2', replace

use "$data\lfs_2017_q3.dta", clear
	gen ky_3t =3
	gen w_p_q = cal_weigh	
	rename c6 c6a
* tempfile file_q3
...
/*
use "$data\lfs_2016_q4.dta", clear
	gen ky_3t =4
	gen w_p_q = cal_weigh
	gen thieuvieclam= thieuvieclamthongthuong1
	gen tinh = matinh
	gen c6a = 1 if  hdkt  ==1 | hdkt ==2 | hdkt ==3		// Chu y  truong hop nay
	replace c6a = 2 if  hdkt ==4
append using `file_q1'
append using `file_q2'
...
*/
gen underemployment = thieuvieclamthongthuong
...
#delimit cr
*/
do "F:\Dropbox\dofile\ldvl\support_ldvl2017.do"

gen hoatdong = labour_force
recode hoatdong .=2 if c19 == 2
recode hoatdong .=3 if c19 != 2
#delimit;
...
save "$temp\ldvl_2017_q1-4.dta", replace
